...
//          p2.checkDead();
  ...
  void playMatch( int f )
  {
    ...
    boolean done = false;
    round = 0;
    int goFirst = p1.initative > p2.initative ? 1 : (p1.initative < p2.initative ? 0 : f); //!raddi.checked && f == 1?1:0;
    p1.isP1 = true;
    while ( !done )
    {
      round ++;
